...
public class TravelDataAdapter extends BaseAdapter {
    private Context mContext = null;
    private ArrayList<TravelDataModel> mListData;

    private final LayoutInflater mInflater;

    // Context = 액티비티의 부모 , context는 리소스에 접근하게 해줌 context를 받음 아니면 그냥 class로는 리소스에 접근 불가능
    public TravelDataAdapter(Context context, ArrayList<TravelDataModel> listData) {
        mContext = context;
        mListData = listData;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            TextView  title      = (TextView) convertView.findViewById(R.id.addrTxt);
        ...
        TravelDataModel item = (TravelDataModel) getItem(position); // ListData item = mData.get(position)으로도 가능

        //데이터 설정
        if(!item.equals(null) || !item.equals("")) {
            holder.mImage.setImageResource(item.getmImageRes());
            holder.mTitle.setText(item.getmTabTitle());
        }
